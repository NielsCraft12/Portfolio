...
async function initProjectPage() {
    ...
    await Promise.all([
        ...
        customElements.whenDefined('social-buttons'),
        ...
        if (section.type === 'media') {
            if (section.mediaType === 'image') {
                ...
                if (section.src.includes('youtube.com/embed') || section.src.includes('youtu.be')) {
                    ...
                    iframe.style.maxWidth = '480px'; // Set vertical aspect ratio
                    iframe.style.aspectRatio = '9 / 16'; // Explicitly enforce vertical layout
    ...
    (function () {
        ...
        ready(function () {
            ...
            const cfg = (typeof window !== 'undefined' && window.PROJECT_DATA)
                ? window.PROJECT_DATA
            ...
            function detectMobile() {
                ...
                const isTabletLandscape = isTouchDevice && window.matchMedia('(min-width: 951px) and (max-width: 1366px)').matches;
                return isTouchDevice || isNarrow || isTabletLandscape;
            }

            const isMobile = detectMobile();

            console.log('[Embed Debug] embedEnabled:', embedEnabled, 'playableOnMobile:', playableOnMobile, 'isMobile:', isMobile);

            if (!embedEnabled) {
                console.error('[Embed Debug] Embed is disabled.');
            } else if (isMobile && !playableOnMobile) {
                console.warn('[Embed Debug] Mobile detected, embed not playable.');
            } else {
            ...
            if (!embedEnabled) {
                ...
                // Apply vertical aspect ratio for mobile when playableOnMobile is true
                if (isMobile && playableOnMobile) {
            ...
            function handleResize() {
                const isNowMobile = detectMobile();

                if (embedEnabled && isNowMobile && !playableOnMobile) {
                    messageEl.style.display = 'flex';
                    embedSection.style.display = 'none';
                    ...
                    if (iframe && container && isNowMobile && playableOnMobile) {
                        ...
                        iframe.style.margin = '0 auto';
                        iframe.style.display = 'block';
                    } else if (iframe && container && !isNowMobile) {
                        // Reset to default desktop styles (responsive 16:9)
