class ExperienceSection extends HTMLElement {
    ...
    connectedCallback() {
        ...
        setTimeout(() => {
            this.dispatchEvent(new CustomEvent('experience-ready', {
        ...
        const contentHTML = this.experiences.map((exp, index) => {
        ...
        <div class="content-section ${index === 0 ? 'active' : ''}" data-index="${index}">
          <div class="header">
            <div class="title-container">
              ...
              <div class="badges">
                ${exp.badges.map(b => `<div class="skill">${b}</div>`).join('')}
              </div>
            </div>
            <div class="date">${exp.startDate} - <span>${exp.endDate}</span></div>
          </div>
